...
func (site *Site) requiredBatteryMode(batteryGridChargeActive, batteryGridDischargeActive bool, rate api.Rate, sitePower float64) api.BatteryMode {
	...
	case site.batterySolarControl:
		// fork solar control owns watt-level power via the fast loop; keep RS485 enabled
		// (Hold) so applyBatterySolarPower drives every tick. Normal mode would disable
		// RS485 between ticks and hand control back to the inverter. Takes precedence over
		// the upstream grid-discharge mode, an incompatible whole-battery "sell" mode.
		res = keepUnlessModified(api.BatteryHold)
