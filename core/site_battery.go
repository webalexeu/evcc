...
func (site *Site) requiredBatteryMode(batteryGridChargeActive, batteryGridDischargeActive bool, rate api.Rate) api.BatteryMode {
	...
	case site.batterySolarControl:
		...
		// inverter. Takes precedence over optimizer automatic mode too - when both are on,
		// the fast loop stays the single execution path and sources its target from the
		// optimizer's suggestion directly (see batteryAutomaticTarget), rather than the
		// main loop applying a mode here.
