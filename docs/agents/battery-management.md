...
**Interaction with optimizer automatic mode** (`site.Automatic()`, from evcc-io/evcc#32881): `requiredBatteryMode`'s `batterySolarControl` case sits above both `Automatic()` cases, so RS485 ownership is never released just because the optimizer suggests Normal - the two are effectively mutually exclusive per installation. Automatic mode only gets to actually drive the battery mode while `batterySolarControl` is off.
