...
- **Under-delivery**: the engaged set ACKs its commands (no Modbus error) but can't physically deliver them — a faulted, low-SoC-derated or phase-limited unit. A Modbus ACK can't reveal this (it only confirms the write); the *measured* power can. Because the energy-balance target tracks the persisting grid error (`target ≈ measured + grid`), `target − measured` is the undelivered watts; when it exceeds `fastLoopShortfall` (150W) for `fastLoopShortfallDwell` (4 consecutive ticks ≈ 4s, longer than the inverter ramp) the loop engages a standby. `shortfallTicks` resets on engage so a freshly-engaged unit gets time to ramp before another is added. This complements the swap-time `SetBattery*Power` error path (§15), which only handles *no-ACK* failures during a swap.
