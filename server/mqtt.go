...
func (m *MQTT) listenSiteSetters(topic string, site site.API) error {
	for _, s := range []setter{
		...
		{"batterySolarControl", boolSetter(site.SetBatterySolarControl)},
		{"batteryCalibrationCharge", boolSetter(site.SetBatteryCalibrationCharge)},
		{"batteryControlDeadBand", floatSetter(site.SetBatteryControlDeadBand)},
